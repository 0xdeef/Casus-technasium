...
start = True # Dit zorgt ervoor dat de while loop kan beginnen omdat start gelijk aan True moet zijn anders start die niet. 
vraagMinimaal = False # Dit zorgt ervoor dat de while loop voor het uitrekenen alleen pas begint wanneer de gebruiker aangeeft dat hij of zij dit wilt berekenen.
minimalAanwezig = False # Dit zorgt ervoor wanneer de berekening heeft plaatsgevonden dat deze ook word geprint in het document. 
...
while start == True: # Zolang start gelijk is aan True (start == True) gaat die alles wat hieronder staat herhalen totdat start =/= True oftewel start == False
    ...
    if vraagCijfer < 1 : # Dit kijkt als het cijfer onder een 1.
        print("Cijfer moet minmaal een 1 zijn, probeer opnieuw") # Als bovenstaande uitspraak klopt (vraagCijfer < 1) print dan "ijfer moet minmaal een 1 zijn, probeer opnieuw"
    elif vraagCijfer > 10: # Dit kijkt als het cijfer boven een 10.
        print("Cijfer moet maximaal een 10 zijn, probeer opnieuw") # Als bovenstaande uitspraak klopt (vraagCijfer > 10) print dan "Cijfer moet lager zijn dan 10, probeer opnieuw"
    else: # Als bovenstaande uitspraak niet klopt doe dan dit 
        lijstCijfers.append(vraagCijfer) # Met append die je de ingevoerde cijfers aan de lege lijst toevegen die we bovenaan hebben gedefineerd
        ...
        if vraagDoorgaan == "NEE": # Als bovenstaande uitspraak gelijk is aan "NEE" doe dan het volgende 
            start = False # Zet de waarde van start naar False. Dit zorgt ervoor dat de while loop van het begin stopt zodra we klaar zijn met deze loop.
            ...
            if minimaalUitrekenen == "JA": # Als bovenstaande waarde gelijk is aan "JA" voer dan onderstaande uit
                vraagMinimaal = True # Zet de waarde voor vraag minimaal op True. Deze word op een later moment gebruikt 
...
bestandsLocatie = input("Waar wil je het document opslaan? : ") #Hier word de gebruiker gevraagd voor de bestandlocatie bv. C:\Users\JohnDoe\Desktop
...
if  vraagMinimaal == False: # Als de gebruiker heeft gekozen voor niet het minimaal berekenen van aantal cijfers dan doe onderstaande instructies
...
if vraagMinimaal == True: # Als de gebruiker wel heeft gekozen voor het berekenen van minimaal cijfer voor x komende toetsen voer onderstaande instructies uit
    aantaalcijfers = len(lijstCijfers) + vraagAantalMinmaal # Voeg het al ingvoerde aantal cijfers + verwachte aantal cijfers bij elkaar op om te weten hoeveel cijfers er intotaal zijn.
    aantalPunten = aantaalcijfers * 5.5 # Vervolgens het aantal toetsen * 5.5
